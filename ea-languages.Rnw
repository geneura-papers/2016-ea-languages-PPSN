...
The Linux system we have used for testing runs the {\tt
 3.13.0-34-generic \#60-Ubuntu SMP} kernel on an {\tt Intel(R) Core(TM)
  i7-4770 CPU @ 3.40GHz} CPU. Results for bitflip mutation and OneMax
are shown in Figures \ref{fig:time} and \ref{fig:time:mo}. Crossover
results are not shown for brevitym but in general they follow the same
pattern. These figures mainly show how time needed to
...
The first ranking for genetic operators still shows Java as the first,
on average, but also Go as the second and forth, depending on the data
structure, with C\# as 3rd and Kotlin as fifth. Java and C\# repeat
in the top 5 for OneMax; however, functional languages Haskell and
Clojure are ranked 2nd to 4th. 
