...
%Pablo: world->community?
popular or emerging languages, especially if they are not compiled;
%Pablo: I would suggest not to use semi-colon here, but a connector ()
%And also 'specially' means specific purpose (Ex: javascript was designed specially for web), and especially means 'particularly' (I love games, especially the adventure ones)
implementation details and data structures have received 
some more attention, but also limited to or within the realm of
particular languages. %Pablo: received some more attention with respect to what?
...
is indeed basis to consider Java the fastest language, but
%Pablo: this is the first time you mention Java in the abstract, should you change it for "compiled languages (such as Java)" instead?
there are other languages, including interpreted ones, that can hold
its ground against it. 
\end{abstract}
...
benchmarks, an Evolutionary Algorithm (EA) application will not spend the majority of the time
...
%Pablo: you->users or something like that ('you' is too informal)
...
relative speeds across all tests. Besides adding more languages and
implementations, we do %Pablo: this sentence is not clear to me. Besides X we do the same and X? I suggest to change with: This is what we do in this paper, extending the analysis of the previous paper with more languages and...
exactly that in this paper, reproducing the results obtained there and
adding new languages. Next we will explain how the experiment was set up and
...
\item {\em Vector of Boolean values}: not all languages have a
  ...
  into bytes for representing vector of bits, with 32 bits packed in a
...
tested only for Onemax. This list includes 9 out of 10 languages from top 10 of 
...
functions, and substracts it from the total number of languages
...
over lenguages not designed with that feature, such as Java. 
