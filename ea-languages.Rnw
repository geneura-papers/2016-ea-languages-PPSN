...
\ref{sec:res} we 
will present the results of measuring the performance of eleven  % in table \ref{tab:files} there are 23 languages - Pedro
...
In this paper around 30 languages, some of them with several implementations, 
...
Docker container, so that they can be run in particular environments  % maybe a reference to Docker should be added?  - Pedro
and machines.
