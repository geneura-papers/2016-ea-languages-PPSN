...
popular and emerging languages, specially if they are not compiled
languages; implementation details and data structures have received
some more attention, but limited to particular languages.
In this paper we measure the speed executing three common operation in
...
considered for implementing evolutionary algorithms.  
...
result, new implementations, including some made using released
...
running these functions; other like sorting or evaluating complex
fitness function will. However, these functions are well covered by usual
general purpose
...
and second to take real measures and benchmark them to test their
speed and performance relative to other common languages instead of
choosing usual languages blindly based only on past experience. 
...
focused on implementation aspects using C and C++; since then, there
...
evolutionary algorithms almost as fast than Java, but that if took
into account other things like actual speed of writen the code as measured by
...
languages but also new architectures like the Kappa architecture \cite{KappaArch}, microservices \cite{namiot2014micro} or
...
algorithm might be written in different languages, but also new
languages better suited for them. The {\em no fast lunch} principle
...
In this paper, 18 languages, some of them with several implementations, have been chosen for performing all
...
availability of open source implementations for the authors, but we have also
tried to be inclusive in by considering languages that 
represent different philosophies in design as well as languages
...
benchmarks and sizes, we have used the language Julia as a baseline for comparison
...
ggplot( ranks, aes(x=reorder(LanguageData,AvgRank), y=28-AvgRank))+ geom_bar(stat='identity',fill='orange',color='darkblue')+ theme_tufte() + theme(axis.text.x = element_text(angle = 90, hjust = 1))+labs(title='Average rank',x='Language+Data Structure',y='16-average rank')
@ 
