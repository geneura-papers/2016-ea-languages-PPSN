...
% are we using OneMax or Griewank function?   - pedro
% I don't think it's worth the while - JJ
...
\begin{figure}[h!tb]
  \centering
<<results-xo, cache=FALSE,echo=FALSE,fig.width=24,fig.height=24,fig.dpi=300, dev='png'>>=
colourCount = length(unique(measures.bf$languagerepresentation))
getPalette = colorRampPalette(brewer.pal(9, "Set1"))
ggplot(measures.xo,aes(x=length,y=time,colour=factor(languagerepresentation)))+ geom_line() + geom_point() +  ggtitle("Evolutionary algorithm language benchmarks: Crossover")+scale_y_log10()+scale_x_log10()+scale_color_manual(name='Language',values=getPalette(colourCount))+theme( text = element_text(size=36),legend.position="bottom")+guides(colour = guide_legend(title.position = "bottom"))
@
...
\begin{figure}[h!tb]
  \centering
<<results-mo, cache=FALSE,echo=FALSE,fig.width=24,fig.height=24,fig.dpi=300, dev='png'>>=
...
\url{https://git.io/bPPSN16}. %PABLO: replace ANONSERVER with git.io after acceptance!!!
...
\begin{figure*}[h!tb]
  \centering
<<results-ops-ratios,message=FALSE, cache=FALSE,echo=FALSE,fig.width=24,fig.height=24,fig.dpi=300, dev='png'>>=
ggplot(op.ratios,aes(x=reorder(Language, -Ratio, FUN=median),Language,y=Ratio))+ geom_boxplot(notch=TRUE)+ theme( text = element_text(size=36),axis.text.x = element_text(angle = 90, hjust = 1))+scale_y_log10()+labs(x='Language')
@
...
\begin{figure*}[h!tb]
  \centering
<<results-ratios,message=FALSE, cache=FALSE,echo=FALSE,fig.width=24,fig.height=24,fig.dpi=300, dev='png'>>=
ggplot(ratios,aes(x=reorder(Language, -Ratio, FUN=median),Language,y=Ratio))+ geom_boxplot(notch=TRUE)+ theme( text = element_text(size=36),axis.text.x = element_text(angle = 90, hjust = 1))+scale_y_log10()+labs(x='Language')
@
...
\begin{figure*}[h!tb]
  \centering
<<results-ranks-ops,message=FALSE, cache=FALSE,echo=FALSE,fig.width=24,fig.height=24,fig.dpi=300, dev='png'>>=
ggplot( op.ranks, aes(x=reorder(LanguageData,AvgRank), y=25-AvgRank))+ geom_bar(stat='identity',fill='orange',color='darkblue')+ theme_tufte() + theme( text = element_text(size=36),axis.text.x = element_text(angle = 90, hjust = 1))+labs(title='Average rank',x='Language+Data Structure',y='25-average rank')
@
...
\begin{figure*}[h!tb]
  \centering
<<results-ranks,message=FALSE, cache=FALSE,echo=FALSE,fig.width=24,fig.height=24,fig.dpi=300, dev='png'>>=
ggplot( ranks, aes(x=reorder(LanguageData,AvgRank), y=27-AvgRank))+ geom_bar(stat='identity',fill='orange',color='darkblue')+ theme_tufte() + theme( text = element_text(size=36),axis.text.x = element_text(angle = 90, hjust = 1))+labs(title='Average rank',x='Language+Data Structure',y='27-average rank')
@
...
%On the other hand, Go is the best one processing genetic operators
%over binary strings, with Java and C\# also the best among more popular
%languages and PHP and Perl being the best among interpreted languages.
% rewriting the previous sentence (in any case, I do not delete it)  - pedro
...
% ["it is sensible" ?  what is sensible?]
% Sensato, in Spanish. Changed to a good practice - JJ
