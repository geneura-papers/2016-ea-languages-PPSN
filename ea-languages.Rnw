...
% Changed to implementation - JJ
available. Second, to find out the differences in speeds for
%Pablo: what is a range of variation here?
%JJ: difference in speed
...
% eliminated. We'll see if it is necessary later on - JJ
...
with respect to \cite{DBLP:conf/evoW/MereloCBRGFRV16ANONYMOUS}, we
...
ggplot( op.ranks, aes(x=reorder(LanguageData,AvgRank), y=25-AvgRank))+ geom_bar(stat='identity',fill='orange',color='darkblue')+ theme_tufte() + theme(axis.text.x = element_text(angle = 90, hjust = 1))+labs(title='Average rank',x='Language+Data Structure',y='25-average rank')
@ 
...
ggplot( ranks, aes(x=reorder(LanguageData,AvgRank), y=27-AvgRank))+ geom_bar(stat='identity',fill='orange',color='darkblue')+ theme_tufte() + theme(axis.text.x = element_text(angle = 90, hjust = 1))+labs(title='Average rank',x='Language+Data Structure',y='27-average rank')
@ 
...
might be performed using microservices, it is interesting to know
...
microservices approach; in this case, an unit performing fitness
...
throughput so it is economic and sensible to perform this operation
...
as far as the interpreted languages is concerned, both PHP and Perl obtain better results.
Even with these results, often
...
in a canonical evolutionary algorithm working on functions similar to
OneMax; however, a distributed microservices architecture would be
better programmed using Haskell or Clojure for evaluation of binary
fitness functions and Go and even Perl for performing genetic
operators. At any rate, this study shows that it is always sensible to
measure and compare the actual performance of the implementation we
are using and consider the possibility of using different data
structures and even lenguages for our evolutionary algorithm
application. 
...
performance. These measurements will have to be taken in whole system,
